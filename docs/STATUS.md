...
| 23 | الضريبة وsim بعد 10-01 | ⚠️ **قرار مطلوب قبل 2026-10-01** | لا `l10n_sa_edi` ولا CSID ولا استدعاء لفاتورة، ولا فاتورة مرحّلة بعد الحد. لكن sim يرحّل فواتير حقيقية غير موسومة عند «تم التسليم» و«تم الشراء»، وستحمل 15% من 10-01. أربعة خيارات في § 23 أ |
...
13. **صندوق المحادثات، رسائل البوت القبلية (§ 22):** 54 رسالة قبل 09-24 تبقى نصاً فاتحاً على كريمي في الوضع الداكن. تعديلها عبر API مرفوض (403). البديل ملف CSS عام في Odoo (`ir.asset` على `web.assets_web_dark`)، وهو تغيير في واجهة Odoo كلها على tenant مشترك مع prod: ينتظر قرار براء وفحصه بعينه.
14. **تذكير الدفع لا يعمل أصلاً:** `sendPaymentReminders` ([outreach.ts:59](../src/outreach.ts)) يطلب `x_customer_id` و`x_paid_amount` والحالتين `pending`/`partial`، ولا وجود لها في `x_invoice`. عند إصلاحه يُستثنى `x_is_simulation` كما في ملخص التحصيل.
