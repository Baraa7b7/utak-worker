...
2b. **المبيعات** ✅ 2026-09-23 على sim (§ 12). **بقي:** إعادة المحاولة عند HTTP 429 من Odoo.com داخل `src/odoo.ts::call` (كشفه التحقق الحي)، وتوليد PDF لم يُختبر حياً في هذا الأمر (لا متغيرات Gotenberg في بيئة التحقق)، ونشر prod مع قرار ACCOUNTING_SYNC.
