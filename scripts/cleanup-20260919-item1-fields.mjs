...
const f = await call("x_daily_price","fields_get",{attributes:["type","string","relation"]});
for (const [k,v] of Object.entries(f)) {
  if (k.startsWith("x_")) console.log(`  ${k.padEnd(28)} type=${v.type} rel=${v.relation ?? ""} str=${v.string}`);
}
