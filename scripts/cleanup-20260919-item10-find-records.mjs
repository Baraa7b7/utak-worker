// Find existing sale.order / account.move / purchase.order records to test PDF endpoints.
...
const so = await call("sale.order","search_read",{domain:[],fields:["id","name","state","partner_id","amount_total","create_date"],order:"id desc",limit:5});
console.log(`sale.order (latest 5):`); for (const r of so) console.log(`  ${JSON.stringify(r)}`);

const po = await call("purchase.order","search_read",{domain:[],fields:["id","name","state","partner_id","amount_total"],order:"id desc",limit:5});
console.log(`\npurchase.order (latest 5):`); for (const r of po) console.log(`  ${JSON.stringify(r)}`);

const inv = await call("account.move","search_read",{domain:[["move_type","in",["out_invoice","out_refund"]]],fields:["id","name","state","partner_id","amount_total"],order:"id desc",limit:5});
console.log(`\naccount.move (customer invoices, latest 5):`); for (const r of inv) console.log(`  ${JSON.stringify(r)}`);
