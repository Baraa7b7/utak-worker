// Read the SALE_PDF_DOWNLOAD_TOKEN from existing Server Action 971's code.
...
const rows = await call("ir.actions.server","read",{ids:[971],fields:["id","name","code"]});
if (!rows[0]) { console.error("no row 971"); process.exit(1); }
const code = rows[0].code;
const m = code.match(/token=([a-f0-9]{32,128})/i);
if (!m) { console.error("token not found in code:", code); process.exit(1); }
console.log(m[1]);
