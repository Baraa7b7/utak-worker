// Discover x_whatsapp_template fields to find the right status/category cols.
...
console.log(`x_whatsapp_template fields (${fields.length}):`);
for (const f of fields) console.log(`  ${f.name} (${f.ttype}) — ${f.field_description}`);

const row = await call("x_whatsapp_template","read",{ids:[46],fields:fields.map(f=>f.name)});
console.log("\nRow 46:", JSON.stringify(row[0], null, 2));
