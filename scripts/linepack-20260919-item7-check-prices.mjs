// Check x_daily_price for tomato and watermelon today.
...
const prices = await call("x_daily_price","search_read",{
  domain:[["x_product_tmpl_id","in",[71,104]]],
  fields:["id","x_date","x_product_tmpl_id","x_packaging_id","x_price_sar","x_sale_price"],
  order:"x_date desc, id desc",
});
console.log(`prices: ${prices.length}`);
for (const p of prices) console.log(`  ${JSON.stringify(p)}`);
