// § 41 (2026-09-26) — the strict schema gate of tests/s41.test.mts:
// fields_get (read-only) of every model the § 41 parts reads or writes,
// with the selection values. Run after each part's --apply; a model that does
// not exist yet is left out.
//
//   node scripts/s41-20260926-fields-fixture.mjs
//
// Out: tests/fixtures-odoo-fields-20260926-s41.json
...
const out = {
  _source: `fields_get on utakfresh.odoo.com, ${new Date().toISOString().slice(0, 16)}Z (read-only), § 41 (pre-launch fixes + full-day simulation): every field of each model, and the selection values.`,
  _selections: {},
};
...
writeFileSync(new URL("../tests/fixtures-odoo-fields-20260926-s41.json", import.meta.url), JSON.stringify(out, null, 1) + "\n");
