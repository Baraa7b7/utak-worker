// Read-only Cloudflare API snapshot (STATUS § 38, الدفعة 3): deployed versions of prod and
// sim, both schedules, and the sim worker's plain-text vars. GET only; the
// wrangler OAuth token is read from its config and never printed.
//
//   node scripts/wa-b3-20260926-cf-read.mjs <label>
//
// Out: scripts/artifacts/wa-b3-20260926-cf-<label>.json
...
const deployed = async (name) => {
  const r = await api(`/accounts/${account}/workers/scripts/${name}/deployments`);
  const d = (r.deployments ?? [])[0];
  return d ? { created_on: d.created_on, versions: (d.versions ?? []).map((v) => `${v.version_id} ${v.percentage}%`) } : null;
};
...
const out = {
  ...
  prodDeployed: await deployed("utak-worker"),
  simDeployed: await deployed("utak-worker-sim"),
  prodSchedules: await sched("utak-worker"),
  simSchedules: await sched("utak-worker-sim"),
  simVars,
};
writeFileSync(new URL(`./artifacts/wa-b3-20260926-cf-${label}.json`, import.meta.url), JSON.stringify(out, null, 2) + "\n");
